...
- `yarn deadcode` (knip, configured via root `knip.json`) finds unused files/exports/dependencies across all workspaces plus `supabase/functions`. Known false positives, don't re-flag: anything used only internally within its own file (e.g. `varsDark`/`varsSurface` in `constants/visualSystem.ts`, `corsHeaders` in `_shared/cors.ts`) — knip flags "no external importer," not "actually dead." `supabase/functions/_shared/constants.ts`'s unused constants are a deliberate manual mirror of `packages/shared/src/constants.ts` (Deno can't resolve workspace packages) — don't trim them just because a given constant isn't used by any function yet. `_shared/notifications.ts` findings are real but off-limits per the rule above — never remove without explicit instruction.
