...
- **Schedule management** — Calendar/List toggle (persisted); calendar shows 14-day slot grid with booked slot overlays (client name, service); list view shows all upcoming bookings; tapping any booking opens a detail bottom sheet
  ...
  - **Day navigation** — ◀ date label ▶ arrow header. Fixed-width label shows "Today" on effective today, otherwise "Weekday, DD Mon" (e.g. "Wednesday, 21 Jun"). Tapping the label opens a monthly `react-native-calendars` grid modal; days outside the 14-day editable window are disabled; fully-blocked days show a red dot. Non-today view shows a "Go to today" button alongside the block controls.
