...
- **Schedule management** — Calendar/List toggle (persisted); calendar shows 14-day grid with booked slot overlays (client name, service, status dot); list view shows all upcoming bookings; tapping any booking opens a detail bottom sheet
  ...
  - Slot states: green ✓ (available), red ✕ (blocked), green ⚡ (auto-accept active), thick black fill (booked); legend has "What the slots mean" heading and "Tap any slot to block or unblock it" hint
  - **Block / Unblock a range** — "Block a range" link in the legend footer opens a bottom sheet with Block | Unblock mode toggle, From/To 30-min time chip pickers (08:00–22:00), Mon–Sun day-of-week chips (multi-select), and a Repeat selector: "Just this week" or "Until a date" (flash-calendar date picker, capped at 8 weeks ahead). Live preview shows "up to X slots". On confirm, batch-writes or batch-deletes `unavailable` rows in `vendor_calendar`; skips active bookings and `transport_buffer` rows. A 10-second undo toast appears after every action — tapping Undo reverses the exact rows written (block undo deletes inserted rows and reverts any auto_accept rows; unblock undo re-inserts deleted rows).
