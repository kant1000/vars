...
| Current milestone | 410 Vendors in the Pipeline (June 2026, complete). App Store Launch: end of July 2026 (target — Apple Developer enrollment currently stalled, see `docs/store-submission-tracker.md`). Vendor Onboarding: September 2026. Both Sides Open (customer marketing go-live): 1 October 2026. |
...
| Auth | Supabase Auth — customers: WhatsApp phone number + OTP only (Google/Facebook removed, email is optional metadata); vendors: email (password login, OTP for new signups) or phone (always OTP) |
...
- **Vendor profile** — compact side-by-side header (72px avatar + name / rating / badges / bio / "Typically accepts in X"); portfolio photo carousel underneath (approved photos only, tap to expand via lightbox); sticky Services | Reviews tab row; Services visible by default; swipe left/right on content to switch tabs; sticky "Book for ₦X,XXX" CTA once at least one service is selected. Legal name and phone number are fetched via `get_vendor_reveal_state` and only render once that customer's booking with this vendor has reached "on my way" (payment captured); a pending-loader state shows before then, live-updated via Realtime.
...
- **Live tracking** — map subscribes to Supabase Realtime on the vendor row for instant position updates while en route, with a 60-second poll as a fallback only (for a silently-dropped Realtime channel, e.g. after backgrounding); shows "Last seen Xm ago" instead of "Live" once the last update passes 2 minutes, since vendor position only updates while the vendor's own app is foregrounded. Full access details revealed 15 minutes before appointment. Vendor phone number and legal name are separately gated behind `get_vendor_reveal_state` (fires once the booking reaches "on my way" / payment captured, persists thereafter for that customer+vendor pair) — the phone number only displays once both that gate and the 15-min countdown have passed.
...
- **Jobs dashboard** — incoming requests with 1-hour accept window; active jobs with flow buttons (On My Way → Arrived → Service Rendered); cancel button for accepted/in-progress bookings; history. Tapping "On My Way" opens a **gate payment modal** (confirm → charging → success/awaiting_payment/error) — outside taps and Android back are blocked during the charging stage; the modal auto-closes on success (1500ms) and shows a retry or cancel option on error. The booking only advances to `on_way` after the payment cycle completes.
...
| `20260816000005_backfill_kyc_verified_at` | Backfills `kyc_verified_at` for already-verified vendors where it was never actually set on verification (webhook bug — fixed same day) |
| `20260816000006_legal_name_reveal` | Adds `legal_name_revealed BOOLEAN` to the customer↔vendor relationship, frozen against direct client writes the same way `gate_fired` already is |
| `20260816000007_vendor_reveal_state_rpc` | Creates `get_vendor_reveal_state(p_vendor_id)`, a `SECURITY DEFINER` RPC that returns legal name + phone number only once the calling customer's booking with that vendor has reached "on my way" (payment captured); persists once revealed for that pair |
| `20260816000008_backfill_legal_name_revealed` | Backfills `legal_name_revealed = true` for customer+vendor pairs that had already passed the reveal point before migration `20260816000007` shipped |
| `20260816195403_cross_role_phone_uniqueness` | Original cross-role phone uniqueness trigger (superseded in mechanism by `20260816201838_phone_identity_registry`, same day) |
...
| `vendors` | Vendor accounts with KYC, ratings, zone settings, identity image columns (`profile_image_url`, `profile_image_raw_url`, `profile_image_locked`), `kyc_submitted_at` (real "a verification attempt happened" signal, distinct from `kyc_status` which defaults to `pending` at row creation — used for onboarding-resume routing and the admin "Pending KYC" stat), `kyc_nin_hash`, and recurring schedule rules (`recurring_block_weekdays integer[]`) |
...
| `vendor-kyc-init` | POST | Generates a Youverify Passive Liveness session server-side (requires the secret API key: separate `session/generate` and `liveness/token` calls) and returns a URL to the `vendor-kyc-widget` page (opened in-app WebView) carrying the session credentials. Not a hosted verification link — Youverify has no such API; their product is a client-side SDK widget. Rejects re-initiation on an already-`verified` vendor. Does not touch `kyc_submitted_at` (that's set by `vendor-kyc-verify`, once a real attempt happens). |
| `vendor-kyc-verify` | POST | Called after the widget captures a selfie. Checks the selfie + a vendor-entered NIN against Youverify's synchronous NIN eIDV endpoint (`POST /v2/api/identity/ng/nin`), computes `kyc_nin_hash` (HMAC of the NIN, keyed by `NIN_HASH_PEPPER` — fails closed if unset), then calls `vendor-kyc-webhook` internally with a payload shaped like its expected async callback, reusing its storage/notification/is_active logic rather than duplicating it. The raw NIN is never stored. |
| `vendor-kyc-webhook` | POST | Receives Youverify result — clean pass: `kyc_status = verified` + `is_active = true` (instant activation); also extracts the liveness face image from the payload, uploads the raw original to the private `vendor-identity-raw` bucket (path stored in `profile_image_raw_url`) and a passport-cropped 400×400 JPEG to the public `vendor-identity-images` bucket (URL stored in `profile_image_url`), sets `profile_image_locked = true`. Image failure is non-blocking — KYC pass completes regardless. Failure: `kyc_status = rejected`, appears in admin queue. Before finalizing a verified pass, checks the incoming `kyc_nin_hash` (computed by `vendor-kyc-verify` from the submitted NIN + `NIN_HASH_PEPPER`) against `vendors.kyc_nin_hash` for any other `verified` vendor; a collision (or a `23505` unique-violation race on the concurrent-write backstop) routes the account to `needs_review` instead, with generic copy — never revealing that the NIN is already registered. A payload missing `nin_hash` is also routed to `needs_review`. |
| `migrate-raw-kyc-images` | POST (one-time ops) | Moves legacy raw KYC liveness images from the public `vendor-identity-images` bucket to the private `vendor-identity-raw` bucket and updates `profile_image_raw_url` from the full public URL to the storage path. Idempotent — skips vendors already using storage paths. Auth: `Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>`. Run only if `SELECT id FROM vendors WHERE profile_image_raw_url LIKE 'http%'` returns rows. |
...
2. Fetches the image and uploads the raw original to the **private** `vendor-identity-raw/{vendor_id}/raw.jpg` bucket — the storage path (not a full URL) is stored in `profile_image_raw_url`
3. Crops a passport-style square (centre, top 65% of frame → 400×400 JPEG) and uploads it to the **public** `vendor-identity-images/{vendor_id}/profile.jpg` bucket — the public URL is stored in `profile_image_url`
4. Sets `profile_image_url` (cropped public URL), `profile_image_raw_url` (storage path in private bucket), and `profile_image_locked = true` on the vendor row
...
Raw biometric images are NDPA-sensitive and are stored in the private `vendor-identity-raw` bucket. The admin vendors panel generates server-side signed URLs (3600s TTL via `createSignedUrls`) to display the raw audit image — the storage path is never set as an image `src` directly. Legacy vendors whose `profile_image_raw_url` is a full `http://` URL (written by the old webhook before the bucket split) can be migrated using the `migrate-raw-kyc-images` one-time ops function.
