...
- **Outreach Queue** — review and bulk-approve vendor lead messages generated by the nurture cron. Checkboxes + bulk approve/reject with per-message channel selector. Compose panel for manual segment blasts (filter by service type, converted/unconverted toggle). See [Vendor Lead Outreach System](#vendor-lead-outreach-system).
...
| `vendor_leads` | Pre-launch interest signups — `lead_state` (PROSPECT/COLD/VERIFIED/CONVERTED), `pioneer`, `last_outreach` |
