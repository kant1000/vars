-- ============================================================
-- VARS — Migration: vendor_lead_tick — fix VERIFIED transition
-- 2026-08-14
--
-- Both places checking a lead's readiness to become VERIFIED used
-- `converted = false`, which can never be true at that point: converted
-- flips to TRUE the moment a matching vendor account is created (at
-- signup, in transfer_pioneer_from_lead() — 20260705000002+), long
-- before KYC even starts. By the time a lead's linked vendor could ever
-- pass KYC, converted is already true. Fixed both to `converted = true`,
-- matching reality — converted_vendor_id is only ever set alongside
-- converted = true by that trigger, so this is safe and consistent.
--
-- Effect: vendor_leads.lead_state can now actually reach 'VERIFIED'
-- (previously permanently stuck at PROSPECT/COLD for every lead, even
-- ones long since verified — pure data-accuracy fix, e.g. for admin
-- funnel reporting). The Step 2 cleanup DELETE (clearing pending
-- intro/reengagement drafts once a lead is VERIFIED) becomes reachable
-- too, which is desired — no message-generation risk, since go-live
-- message generation was already retired in 20260814000002.
--
...
BEGIN
  ...
  -- Go-live message generation retired (see 20260814000002) — vendor-kyc-webhook
  -- owns the verified notification now, sent instantly at the actual
  -- verification event rather than waiting on this hourly tick.
  DELETE FROM vendor_lead_outreach
  WHERE status IN ('draft', 'approved')
    AND message_type IN ('introduction', 'reengagement')
    AND channel IN ('whatsapp', 'sms')
    AND lead_id IN (
      SELECT id FROM vendor_leads WHERE lead_state = 'VERIFIED' AND converted = true
    );
