-- ============================================================
-- VARS — Migration: vendor_lead_tick — fix stale domain + launch month
-- 2026-08-14
--
-- Two corrections to the hardcoded copy in this function:
--   1. "August" → "September" (real launch is end of September, not August).
--   2. vars.app (never a real hosted domain, dead link) → bookwithvars.com,
--      matching the domain fix already applied to lead-copy.ts this session.
-- No logic changes — same transitions, same guards, same cadence.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    END ||
    'Start accepting bookings: https://bookwithvars.com/go-live',
    'draft'
  FROM verified_leads;
