-- ============================================================
-- VARS — Migration: vendor_lead_tick — September → October
-- 2026-08-14
--
-- Mechanical text swap only, matching lead-copy.ts's LAUNCH_MONTH default
-- change (August → September → October, this session). Customer launch
-- ("Both Sides Open") is 1 October 2026 per the roadmap
-- (apps/landing/src/app/roadmap/data/milestones.ts). No logic changes.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN pioneer
      THEN first_name || ', VARS opens to customers in October and your Pioneer spot is still reserved. First 3 bookings: 0% commission. Set up your ' ||
           CASE service_type
      ...
      ELSE first_name || ', VARS opens to customers in October. Set up your ' ||
           CASE service_type
             WHEN 'barbing'      THEN 'barbering'
             WHEN 'hair_styling' THEN 'hair styling'
             WHEN 'makeovers'    THEN 'makeovers'
  ...
  SELECT
    ...
    'reengagement_email',
    'email',
    'Still thinking, ' || first_name || '? We open in October — don''t miss it',
    ...
    CASE service_type
      ...
      ELSE                     'beauty services'
    END ||
    ' on VARS but haven''t completed your profile yet. We open to customers in October — vendors who complete setup now will be live from day one. KYC takes 2–3 minutes. ' ||
    CASE WHEN pioneer
  ...
  SELECT
    ...
    'whatsapp',
    'INTRO_WA',
    'Hi ' || first_name || '! VARS opens to customers in October — set up your ' ||
    CASE service_type
      WHEN 'barbing'      THEN 'barbering'
      WHEN 'hair_styling' THEN 'hair styling'
      WHEN 'makeovers'    THEN 'makeovers'
  ...
  SELECT
    ...
    CASE WHEN pioneer
      ...
      ELSE first_name || ', get set up before VARS opens in October'
    END,
    'Hi ' || first_name || '! VARS opens to customers in October — set up your ' ||
    CASE service_type
      WHEN 'barbing'      THEN 'barbering'
