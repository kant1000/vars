-- ============================================================
-- VARS — Migration: vendor_lead_tick — reengagement WA service type
-- 2026-06-08
--
-- Fix: reengagement WhatsApp copy was missing service_type label
-- in both pioneer and non-pioneer branches, inconsistent with
-- introduction WA copy. Adds service label ("barbering", etc.)
-- to both branches.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN pioneer
      THEN first_name || ', VARS opens to customers in August and your Pioneer spot is still reserved. First 3 bookings: 0% commission. Set up your ' ||
           CASE service_type
           ...
           END ||
           ' profile before we go live: https://vars.app/activate'
      ELSE first_name || ', VARS opens to customers in August. Set up your ' ||
           CASE service_type
             WHEN 'barbing'      THEN 'barbering'
             WHEN 'hair_styling' THEN 'hair styling'
             WHEN 'makeovers'    THEN 'makeovers'
             ELSE                     'beauty services'
           END ||
           ' profile — vendors who set up now will be first in customer searches. Takes 5 minutes: https://vars.app/activate'
    END,
    'draft'
  FROM cold_leads;
  GET DIAGNOSTICS v_extra_count = ROW_COUNT;
