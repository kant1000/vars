...
3. For each record:
   ...
   - `sms` → Termii `/api/sms/send` with `channel: generic`
     *(SMS records are not currently generated by the tick; this branch is
...
Termii dashboard but are never delivered to the recipient.
...
and approved by Meta via the Termii dashboard. Email and SMS channels are not
