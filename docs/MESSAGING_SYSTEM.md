...
`supabase/functions/_shared/lead-copy.ts`. Framing is "October launch"
(corrected twice 2026-08-14: August → September → October, matching
"Both Sides Open" on the roadmap — see migrations `20260814000001` through
`20260814000004`): leads are told customers start booking in October and
they should set up their profile to be ready from day one.
...
| `introduction` | WhatsApp | `whatsappIntroTemplate()` | 24h after sign-up |
| `reengagement_email` | Email | `reengagementEmail()` + `reengagementEmailHtmlParts()` | 7 days after last outreach |
| `reengagement` | WhatsApp | `whatsappReengagementTemplate()` | 7 days after last outreach |
| KYC verified | WhatsApp + push/in-app | `whatsappGoLiveTemplate()` (reused directly, not via `deliver-outreach`) | Instantly, from `vendor-kyc-webhook` |
| KYC rejected | WhatsApp + push/in-app | `whatsappKycRejectedTemplate()` | Instantly, from `vendor-kyc-webhook` |

`go_live` as a `deliver-outreach`-processed message type is retired (2026-08-14)
— it was generated by `vendor_lead_tick()`, which turned out to be structurally
unreachable (see `docs/DAILY_AUDIT.md` §15). `vendor-kyc-webhook` is now the
sole, instant sender for both the verified and rejected KYC outcomes — see
`docs/DAILY_AUDIT.md` §9 for the full behavior (including the `is_active`
dual-gate against `paystack_subaccount_code`, and the `needs_review` notification
added the same day).
...
booking) — except `vars_vendor_golive` (WhatsApp), which deliberately dropped
the pioneer/earnings line during Meta template submission and is now identical
for every recipient.

The `whatsapp*Template()` functions return `{ name, params }` — a Meta HSM
template name plus positional body parameters — not free text. `deliver-outreach`
sends these as `type: 'template'` 360dialog messages (as does `vendor-kyc-webhook`,
via the shared `sendWhatsAppTemplate()` in `notifications.ts`), since
business-initiated WhatsApp sends can't be free-form. The template names
(`vars_vendor_intro`, `vars_vendor_reengagement`, `vars_vendor_golive`,
