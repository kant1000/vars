...
| App appearance | Light and dark appearances are approved for the mobile app visual-system overhaul. The default control model is system appearance, with a manual override available from both customer and vendor settings. Light mode preserves the monochrome white shell. Dark mode uses the splash screen's black VARS surface as the brand reference, but resolves depth through fixed dark surface steps rather than carrying light-mode shadows across unchanged. |
