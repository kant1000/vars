...
| Element | Decision |
|---|---|
| Palette | Monochrome shell. Core tokens: `ink` `#111111`, `white` `#FFFFFF`, `inkMuted` `#6B7280`, `inkFaint` `#D0D0D0`. Accent tokens: `accentBlue` `#0A7AFF`, `accentAmber` `#F59E0B`, `accentGreen` `#22C55E`, `accentRed` `#DC2626` (darkened from `#EF4444` for AA text contrast, 2026-07-17 — see `docs/audit/mobile.md` §7.1). Originally scoped to glyphs only; superseded 2026-07-18 by the theme-wiring pass, which also uses these as button fills/borders (e.g. `accentBlue`-filled accept buttons, `accentRed`-bordered destructive actions) — see the "Accent fills" row below. All tokens have light/dark variants in `apps/mobile/constants/visualSystem.ts` (`ink`/`inverseInk` fully invert between themes; accent tokens shift to a lighter/more saturated variant in dark mode rather than inverting). The older `apps/mobile/constants/colors.ts` `Colors` object duplicates a static, non-theme-reactive subset for brand/semantic values (status colors, pioneer gold) that intentionally don't change with theme — see "Deliberate non-migration boundary" below. |
| Accent fills | Accent tokens (`accentBlue`, `accentRed`, `accentGreen`) are used as button fills and borders where a semantic action calls for it (destructive delete/cancel, primary confirm on a gate/payment action), not just small glyphs. When used as a fill, pair the text/icon with `theme.color.inverseInk` if the fill is `theme.color.ink`, or a fixed light color if the fill is an accent token (accents don't fully invert in dark mode, so a static light icon/text usually stays legible on both variants — verify contrast if introducing a new pairing). Decided during the theme-wiring pass, 2026-07-18. |
...
| Primitive catalogue | Fixed set of 10 components — `VarsSurface`, `VarsButton`, `VarsInput`, `VarsCheckbox`, `VarsSwitch`, `VarsSegmentedControl`, `VarsTabItem`, `VarsSkeleton`, `VarsToast`, `VarsIcon` — is the only sanctioned way to build new UI in `apps/mobile`. Per-primitive use / do-not-use guidance in the [Primitive catalogue reference](#primitive-catalogue-reference) below; source in `apps/mobile/components/ui/primitives.tsx`. Shipped and live on vendor onboarding (all 5 steps), booking flow, gate checkout, schedule (undo toast + booking sheet), and both appearance-settings screens. A proposed new primitive needs explicit justification for why composing the existing 10 can't do the job. Approved by founder, 2026-07-17. **Known deviation, resolved:** `VarsDialog` was built but never adopted — every confirm/cancel dialog in the app used `apps/mobile/components/ConfirmModal.tsx` instead, a separate component built outside the catalogue. `VarsDialog` has been removed from `primitives.tsx`; `ConfirmModal` is the sanctioned pattern for confirm dialogs going forward. |
| Theme token wiring | Distinct from primitive-catalogue adoption above: every screen and shared component in `apps/mobile` now reads live `theme.color.*` tokens (via `useVarsTheme()` + `useMemo(() => makeStyles(theme), [theme])`) for its own styling, not just forwarding `theme` into a primitive prop. Completed app-wide (~40 files) 2026-07-18 — full phase log in `docs/audit/mobile.md` §10. Remaining static `Colors.*` references are deliberate: per-status semantic maps, brand accent colors (Google/Facebook buttons, pioneer gold), fixed-warning-treatment banners, and fixed-contrast overlays on photos/maps/WebViews. `accentRed`/`error` is unified to `#DC2626` across both the static `Colors` object and the theme tokens — do not reintroduce `#EF4444`. The two muted-text tiers in the static system (`textSecondary` `#6B7280`, `textMuted` `#A3A3A3`) collapse into the single theme token `inkMuted` — do not add a new tier without a founder decision. On-device light/dark toggle walkthrough completed 2026-07-25 on a physical Android device (system toggle, manual override, OLED tonal separation, contrast all confirmed) — iOS still unverified, no Mac/Xcode available. |
...
| Loading animation | Blades open and close slowly and precisely. 0.7s per direction, ease-in-out, no bounce. Three sizes: small (23×24px), medium (39×39px), large (61×63px). `color` prop is a fixed SVG fill (`'light'` \| `'dark'`), not a theme token — the caller must pick per surface, and must flip with `theme.appearance` since `ink` itself inverts between themes. On a `theme.color.bg`/outline surface: `theme.appearance === 'dark' ? 'light' : 'dark'`. Inside a `theme.color.ink`-filled button (paired with `theme.color.inverseInk` text): the inverse, `theme.appearance === 'dark' ? 'dark' : 'light'`. Fixed-brand-fill surfaces (Google button, pioneer-gold banners) keep a static color regardless of theme. Got this wrong at every call site app-wide until the 2026-07-18 fix — see `docs/audit/mobile.md` §10. SVG viewBox must be `"-120 -90 800 920"` — the 920 height gives 112 units of bottom clearance so blade tips never clip at ±32°. The web roadmap `ScissorIcon` uses the same value. Do not reduce VB_H below 920. |
...
| User verification | Customers are NOT KYC'd (no Youverify check), but their phone number IS OTP-verified — it's the only signup/login identifier, synced to `profiles.phone_number` only once WhatsApp OTP confirms it (migration `20260819010001`). | Identity trust is concentrated on the vendor side via Youverify; phone verification on the customer side exists so a vendor can actually reach whoever they're booked with, not as a KYC substitute. |
...
| Auth methods | Customers: WhatsApp phone number + OTP is the only signup/login identifier (Google/Facebook/email-as-identifier removed, `d6a4a41`). Email is optional profile metadata only, never used to sign in. Vendors: email or phone, both OTP-capable, unchanged. | Google/Facebook sign-in was dropped as a product decision (not just cleanup) — see the auth-redesign plan for the funnel-conversion tradeoff this accepted. |
...
| Booking for someone else | `bookings.recipient_name`/`recipient_phone` (migration `20260817012142`) — both null means the booking is for the account holder; a non-null `recipient_name` means the vendor is meeting someone else. The vendor sees the recipient only by default. A collapsed "view original booker's details" link (`get_booking_client_contact` RPC) lets the vendor fall back to the account holder's own name/phone if the recipient turns out to be unreachable, gated behind the same `phone_revealed` flag as the recipient's number. | The person paying and the person receiving the service aren't always the same person — the vendor needs a way to reach whoever actually booked if the recipient doesn't answer, without the account holder's number being exposed by default. |
| Phone-reveal has parity on both sides | Vendor and customer both read phone numbers through a `SECURITY DEFINER` RPC gated on `bookings.phone_revealed` (`get_booking_client_contact` for the vendor, `get_vendor_reveal_state` for the customer) — never a raw column join in a list/detail query. | Closes a real leak: the vendor's booking-list query used to join `recipient_phone`/`profiles.phone_number` directly, so the raw number reached the client regardless of whether `phone_revealed` was true — the render-time gate wasn't a real gate. Fixed alongside the recipient-fallback work above. |
