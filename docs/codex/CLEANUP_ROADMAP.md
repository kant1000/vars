...
- **Schedule calendar overhaul** (`app/(vendor-tabs)/schedule.tsx`): Complete UX rebuild of vendor schedule navigation and blocking. (1) **Day navigation**: replaced 14-chip horizontal scroll strip with a ◀ date label ▶ arrow header; fixed-width label (`minWidth: 200`) shows "Today" or "Weekday, DD Mon"; tapping opens a `react-native-calendars` monthly modal (pure JS — avoids native module crash pattern); days outside the 14-day window are disabled; fully-blocked days show a red dot; `hideExtraDays={true}` prevents greyed-out adjacent-month overflow cells. (2) **Block / Unblock day**: single context-aware button replaces 2-line legend — blocks/unblocks only future unbooked slots; 3-second undo toast (was 10 s); "Go to today" button appears alongside when not on effective today. (3) **Block a range in the modal**: long-press any calendar day starts range selection; Block button appears immediately (single-day range valid); tap a second day extends it; auto-detects block vs unblock from current slot state. Stale-closure fix: `rangeStartRef` updated synchronously in `onDayLongPress` so `onDayPress` reads the correct value before React re-renders. (4) **Recurring weekly blocks**: "BLOCK EVERY" Mon–Sun chip row at the bottom of the calendar modal; tap to add a weekday rule (applies immediately to 14-day window + reapplies to new days on each focus); tap to remove (clears future unbooked blocks for that weekday). Multiple weekdays, no end date. DB: `add_recurring_block_weekdays_to_vendors` migration adds `recurring_block_weekdays integer[] DEFAULT '{}'` to `vendors`. `react-native-calendars` added to mobile deps. Done.
