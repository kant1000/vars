...
- **Vendor auth redesign** (`apps/mobile/app/auth/vendor-login.tsx`): Full rewrite. Email OTP primary (phone OTP greyed out + "Soon" until 360dialog is configured). Flow: enter email → `vendor-check-identity` → has_account (password form + "Send me a code instead") | lead_only (OTP → create password → onboarding) | not_found (error + link to bookwithvars.com). `vars_onboarding_done` set on every successful auth path. `routeToVendorState` resumes at the correct onboarding step using DB state. Done.
...
- Scope support channel for the Settings "Get help" row (email, WhatsApp, or Intercom) — placeholder Alert in place.
