...
- **Pre-production cleanup**: removed dead `_dev-visual-preview` route (zero references anywhere) and unused `VarsDialog` primitive (never adopted — `ConfirmModal` is the de facto standard, catalogue now documented as 10 components). ESLint 23 warnings → 0: unused imports/vars removed outright; `exhaustive-deps` warnings suppressed with an inline comment only where fixing them was either a no-op (`Animated.Value` refs) or carried real behavioral risk (`_layout.tsx` routing effects, the booking flow); `privacy-data.tsx`'s `loadData` got a real fix (wrapped in `useCallback`) since it carried no such risk. Done.
- **Biometric lock enforcement gate**: `lib/useBiometricLock.ts` + `components/BiometricLockOverlay.tsx`, wired additively into `_layout.tsx` (see Immediate Next Steps below — this closes that item). Done.
- **Android build fixed — was an invocation bug, not the Gradle issues**: `npx expo <anything>` fails in this Yarn 1 classic workspace with a doubled-`node_modules` module-resolution error, before any Gradle logic runs — a third, separate, previously undiagnosed problem distinct from the two Gradle bugs already fixed and documented in `docs/audit/mobile.md` §6. Fixed by using `yarn workspace @vars/mobile android` instead; full writeup in `docs/MOBILE_DEVICE_TESTING.md`. Verified: `BUILD SUCCESSFUL`, installed and ran on a physical Galaxy A40. Done.
- **Android API 36 target**: Google Play requires this for new submissions from 31 Aug 2026 (this app's prior effective target was 34, already below the current 35 floor for existing apps). Set via the `expo-build-properties` plugin in `app.config.js` — deliberately not a direct `android/build.gradle` edit, since that folder is gitignored and regenerated on every prebuild/EAS build. Verified: compiles clean, installs, runs. Done.
- **Duplicate `expo` dependency removed** from root `package.json` (`apps/mobile/package.json` already declares it; the two had drifted to different semver ranges under Yarn 1 hoisting). Done.
- **EAS CLI upgraded** 18.5.0 → 21.2.0 globally. Done.
- **Full on-device QA pass — Android**: walked the entire founder device QA checklist (`docs/audit/mobile.md` §9) on a physical device — theme/dark-mode (system toggle, manual override, OLED tonal separation, contrast), skeleton footprints, no-optimistic-success on payment/KYC/dispute/deletion, Material Icons, content truncation, biometric lock, ScissorsLoader (all 3 sizes, correct color per theme), offline banner, booking flow end-to-end, vendor onboarding all 5 steps. Zero fails. iOS not run — no Mac/Xcode available in this environment; the real path forward is `eas build --platform ios` (builds in Expo's cloud, no local Xcode needed) once the Apple Developer account clears verification. Done.

## Phase 2b+++++++++++++ Work Done (August 2026)

- **Vendor location bar + auto-accept zone collapse**: vendors previously set `base_location` exactly once, at onboarding, with no screen to change it afterwards — a mis-pinned or relocated vendor was stuck in the wrong customers' search results permanently. Added an editable location bar to `(vendor-tabs)/profile.tsx`, reusing the same `LocationPicker` component customers already had (GPS or pick-an-area, sticks until changed) rather than inventing a second control. Collapsed the previously-separate `auto_accept_zone_lat`/`auto_accept_zone_lng` pair onto `base_location` — that duplication was the direct cause of the wrong-field transport-fee bug (`991f6d6`), and kept as two fields it would have meant two different location UIs on the vendor side. `auto_accept_zone_radius_km` is now the only zone-specific setting; new edge function `vendor-set-base-location` writes the location and clears `auto_accept_zone_confirmed_date` in the same update, since moving the location moves the zone with it. **This supersedes both Phase 2b++++++++'s `vendor-zone-setup.tsx` entries above (lines 168, 170)**: that screen no longer calls `Location.*` APIs at all — its map marker is now read-only, centred on `base_location`, and the picker (not GPS) is the only way to change it. Onboarding's `step-1-profile.tsx` also switched from its own hand-rolled GPS-and-area-list picker to the same shared `LocationPicker`, removing ~60 lines of duplicated logic. Migration `20260822000001_collapse_zone_onto_base_location` also dropped the long-dead `live_location`/`live_location_updated_at` columns (zero code references, zero populated rows, superseded by `vendor_current_lat/lng`). Full detail: `docs/audit/regression-sweep-2026-08-22.md`. Done.

- **Stale-coordinate payment-gate bug fixed**: `vendor-update-location` only ever wrote `vendor_current_lat/lng` while a booking was `on_way`, never cleared them afterward, and never wrote `vendor_location_updated_at` at all. `send-reminders`' proximity-gate check read those coordinates with a null check only — no staleness check — before calling `paystack-gate`, which charges the customer. A leftover position from a previous job could therefore fire a real payment gate before a vendor had actually set off. Fixed by stamping `vendor_location_updated_at` on every ping and having `send-reminders` reject any position older than the new `VENDOR_LOCATION_MAX_AGE_MINUTES` constant (10 minutes, comfortably above the 60s ping cadence). Found while auditing the location bar work, not the original target of that change. Done.

- **Reschedule pre-transport-buffer bug fixed**: `customer-accept-reschedule` created the post-service transport buffer at the reschedule's new time but never the pre-service travel buffer, even though the booking already carried `pre_transport_buffer_slots` from the original distance calculation (reschedule moves the time, not the distance). `paystack-capture` already did both calls correctly for a non-rescheduled accept; this mirrors that exact pattern. Net effect before the fix: a rescheduled booking for a customer beyond the 5km base radius could leave its vendor booked back-to-back against a job needing 30-60 minutes of travel. Found via a history-driven regression sweep (checking code last touched before a rule-changing migration against what that migration actually changed) rather than by looking for this bug specifically — full method and a retracted false lead in `docs/audit/regression-sweep-2026-08-22.md`. Done.

- **iOS location permission scoped down**: `app.config.js` requested `NSLocationAlwaysAndWhenInUseUsageDescription` ("Always") despite no code anywhere calling `requestBackgroundPermissionsAsync` or running a background location task — every vendor location read is foreground-only, tied to the app being open. Scoped to "When In Use" only, avoiding an avoidable Guideline 5.1.1 App Review risk. Done.
...
- ~~Run the founder device QA checklist (`docs/audit/mobile.md` §9).~~ **Done on Android** (2026-07-25), zero fails — see Phase 2b++++++++++++ above. Still open: a real screen-reader pass, and the same checklist on iOS once a build is possible there.
