...
- `vendors.auto_accept_zone_lat`, `auto_accept_zone_lng`, `live_location`, `live_location_updated_at` must **not** exist (dropped `20260822000001_collapse_zone_onto_base_location`) — FAIL if any of the four are present; `vendors.vendor_location_updated_at` must exist and be nullable `TIMESTAMPTZ` — see §6
- `bookings.grace_cancelled` — declared in `20240101000005_auto_accept.sql` but confirmed **absent** from the live database (found during the regression sweep, `docs/audit/regression-sweep-2026-08-22.md`) and referenced by zero code; grace-period cancellation works entirely off `auto_accept_grace_expires_at`, which does exist. Not a FAIL by itself — this is a known, accepted migration/live divergence — but WARN if any new code starts writing or reading `grace_cancelled`, since it would silently no-op against a column that isn't there.
...
**Source:** `supabase/functions/paystack-initialize/index.ts`, `supabase/functions/vendor-update-location/index.ts`, `supabase/functions/vendor-set-zone/index.ts`, `supabase/functions/vendor-confirm-zone/index.ts`, `supabase/functions/vendor-set-base-location/index.ts`, `supabase/functions/vendor-cancel-booking/index.ts`, `supabase/functions/customer-accept-reschedule/index.ts`, `supabase/functions/send-reminders/index.ts`, `supabase/functions/_shared/calendar.ts`, `supabase/migrations/20260822000001_collapse_zone_onto_base_location.sql`

- **Zone centre is `base_location`** (collapsed 22 Aug 2026 — was a separate `auto_accept_zone_lat`/`auto_accept_zone_lng` pair, which caused a wrong-field transport-fee bug, `991f6d6`). `vendors.auto_accept_zone_lat`, `auto_accept_zone_lng`, `live_location`, `live_location_updated_at` must **not** exist — FAIL if any of these four columns are reintroduced. `auto_accept_zone_radius_km` is the only zone-specific setting left; "zone configured" means *radius is set*, not *pin is set* — FAIL if any code still treats a null radius with a non-null legacy lat/lng as configured.
- Vendors change their location via `vendor-set-base-location` (profile screen) or the picker inside `vendor-zone-setup.tsx` — both call the same edge function, which writes `base_location` and nulls `auto_accept_zone_confirmed_date` in the same update — FAIL if a location change doesn't also clear the day's confirmation, since that would let auto-accept fire around a centre the vendor never confirmed.
- Firing conditions: `auto_accept_enabled` true AND user within `auto_accept_zone_radius_km` of `base_location` AND `auto_accept_paused_due_to_drift = false` AND `auto_accept_zone_confirmed_date` matches today **or** the booking date AND the slot is actually free — FAIL if the zone-confirmation check is tightened back to "today only" without also accepting the booking date, or if the slot-availability check is dropped. `paystack-initialize` fetches `base_location` once (already needed for the transport fee) and passes it into `checkAutoAccept` — FAIL if the auto-accept check re-fetches location separately, since that reopens the door to the two values drifting apart.
- Drift threshold: `zone_radius + 3km` from `base_location` — FAIL if `+1km` or any flat constant is used instead of the relative formula, or if drift is measured against anything other than `base_location`.
...
- Transport buffers — two kinds, both use `transport_buffer_source_booking_id` FK and are deleted by the same cleanup on any cancellation:
  ...
  - Three call sites create these, not two: `paystack-initialize` (auto-accept), `paystack-capture` (manual accept), and `customer-accept-reschedule` (reschedule accept, fixed 22 Aug 2026 — previously only created the post-booking pair, silently dropping the pre-booking travel buffer at the new time) — FAIL if any of the three creates only one of the pair when `pre_transport_buffer_slots > 0`
- Zone confirmation: maximum once per day (`auto_accept_zone_confirmed_date = today`)
- `vendor-update-location` writes `vendor_current_lat`/`lng` whenever the vendor has an active `on_way` booking (client-driven cadence, not server-enforced every 60s); the drift flag itself is recalculated on every call regardless of booking status — FAIL if the location write is missing from the `on_way` path. Since 22 Aug 2026 it also stamps `vendor_location_updated_at` in the same update — FAIL if that timestamp is missing while `vendor_current_lat`/`lng` are set, and FAIL if `send-reminders`' proximity-gate check (which calls `paystack-gate` and charges the customer on a match) ever reads those coordinates without checking `VENDOR_LOCATION_MAX_AGE_MINUTES` staleness first — coordinates persist after a job ends, so an unbounded read can fire a payment gate on a leftover position from a previous booking.
- **Auto-accept must never touch payment timing.** Verified 22 Aug 2026 by tracing the full path: `paystack-initialize` makes zero Paystack calls regardless of `auto_accepted` and inserts every booking with `gate_fired` defaulting `false`; `vendor-update-job-status` explicitly refuses the `on_way` transition ("use paystack-gate for on_way"); `paystack-gate` (the only place `chargeAuthorization`/`initializeTransaction` is called) has zero references to `auto_accepted` anywhere — it only checks `status = accepted`, the gate window, and an atomic `gate_fired` claim. FAIL if `auto_accepted` ever appears in `paystack-gate` or if any function other than `paystack-gate` sets `gate_fired = true`.
