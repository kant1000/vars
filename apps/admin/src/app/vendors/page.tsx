...
async function getVendors(status: string, page: number, q: string) {
  ...
  let query = db
    .from('vendors')
    .select('id, full_name, phone_number, kyc_status, avg_rating, total_reviews, badge_vars_choice, badge_top_rated, created_at, is_online, cancellation_flagged, settlement_on_hold, is_suspended, profile_image_url, profile_image_raw_url')
